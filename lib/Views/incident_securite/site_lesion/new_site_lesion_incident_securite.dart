...
class NewSiteLesionIncidentSecurite extends StatefulWidget {
  ...
  const NewSiteLesionIncidentSecurite({Key? key, required this.numIncident})
      : super(key: key);

  @override
  State<NewSiteLesionIncidentSecurite> createState() =>
      _NewSiteLesionIncidentSecuriteState();
}

class _NewSiteLesionIncidentSecuriteState
    extends State<NewSiteLesionIncidentSecurite> {
                        ...
                        Visibility(
                            ...
                            child: DropdownSearch<SiteLesionModel>(
                              ...
                              onChanged: (data) {
                                ...
                                debugPrint(
                                    'site Lesion: $siteLesion, code: ${selectedCodeSiteLesion}');
                              },
                              dropdownBuilder: _customDropDownSiteLesion,
                              popupItemBuilder:
                                  _customPopupItemBuilderSiteLesion,
                              validator: (u) => u == null
                                  ? "Site Lesion ${'is_required'.tr}"
                                  : null,
                            )),
                        SizedBox(
                          height: 20.0,
        ...
        if (connection == ConnectivityResult.none) {
          ...
          await IncidentSecuriteService().saveSiteLesionByIncident({
            ...
            ShowSnackBar.snackBar(
                "Successfully", "Site Lesion added", Colors.green);
            //Get.back();
            Get.to(SiteLesionIncidentSecuritePage(
              numIncident: widget.numIncident,
            ));
    ...
    try {
      ...
      if (connection == ConnectivityResult.none) {
        ...
        var response = await LocalIncidentSecuriteService()
            .readSiteLesionIncSecARattacherByIncident(widget.numIncident);
        ...
        await IncidentSecuriteService()
            .getSiteLesionIncidentSecurite(widget.numIncident, matricule)
            .then((resp) async {
  ...
  Widget _customDropDownSiteLesion(
      BuildContext context, SiteLesionModel? item) {
    if (item == null) {
      return Container();
    } else {
      return Container(
