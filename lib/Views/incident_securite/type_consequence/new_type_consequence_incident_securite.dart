...
class NewTypeConsequenceIncidentSecurite extends StatefulWidget {
  ...
  const NewTypeConsequenceIncidentSecurite(
      {Key? key, required this.numIncident})
      : super(key: key);

  @override
  State<NewTypeConsequenceIncidentSecurite> createState() =>
      _NewTypeConsequenceIncidentSecuriteState();
}

class _NewTypeConsequenceIncidentSecuriteState
    extends State<NewTypeConsequenceIncidentSecurite> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          "${'new'.tr} Type Consequence Incident",
  ...
  Future<List<TypeConsequenceModel>> getTypeConsequence(filter) async {
    try {
      List<TypeConsequenceModel> _typeList =
          await List<TypeConsequenceModel>.empty(growable: true);
      List<TypeConsequenceModel> _typeFilter =
          await List<TypeConsequenceModel>.empty(growable: true);
      var connection = await Connectivity().checkConnectivity();
      if (connection == ConnectivityResult.none) {
        ...
        var response = await LocalIncidentSecuriteService()
            .readTypeConsequenceIncSecARattacher(widget.numIncident);
        ...
        await IncidentSecuriteService()
            .getTypeConsequenceIncSecARattacher(widget.numIncident, matricule)
            .then((resp) async {
  ...
  Widget _customDropDownTypeConsequence(
      BuildContext context, TypeConsequenceModel? item) {
    if (item == null) {
      return Container();
    } else {
      return Container(
