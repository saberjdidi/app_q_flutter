...
class TypesCausesActionPage extends StatefulWidget {
  ...
  const TypesCausesActionPage({Key? key, required this.idAction})
      : super(key: key);
...
class _TypesCausesActionPageState extends State<TypesCausesActionPage> {
  ...
  List<TypeCauseModel> listTypesCauses =
      List<TypeCauseModel>.empty(growable: true);
  ...
  void getData() async {
    try {
      ...
      if (connection == ConnectivityResult.none) {
        var response =
            await localService.readTypeCauseActionById(widget.idAction);
        ...
        //rest api
        await ActionService()
            .getTypesCausesOfAction(matricule, widget.idAction, 1)
            .then((resp) async {
        ...
        floatingActionButton: FloatingActionButton(
          onPressed: () {
              ...
              try {
                ...
                if (connection == ConnectivityResult.none) {
                  ...
                  var response = await LocalActionService()
                      .readTypeCauseActionARattacherById(widget.idAction);
                  response.forEach((data) {
                    var model = TypeCauseModel();
                    model.codetypecause = data['codetypecause'];
                    model.typecause = data['typecause'];
                  ...
                  await ActionService()
                      .getTypesCauseActionARattacher(matricule, widget.idAction)
                      .then((resp) async {
                    resp.forEach((data) async {
                      var model = TypeCauseModel();
                      model.codetypecause = data['codeTypeCause'];
            ...
            Widget _customDropDownTypeCause(
                BuildContext context, TypeCauseModel? item) {
              if (item == null) {
                return Container();
              } else {
                return Container(
                            ...
                            children: [
                              ...
                              Center(
                                child: Padding(
                                  ...
                                  child: Center(
                                    child: Text(
                                      'New Type Cause of Action N°${widget.idAction}',
                                      style: TextStyle(
                                            ...
                                            if (_addItemFormKey.currentState!
                                              ...
                                              try {
                                                ...
                                                if (connection ==
                                                  ...
                                                  int max_type_cause =
                                                      await LocalActionService()
                                                          .getMaxNumTypeCauseAction();
                                                  var model = TypeCauseModel();
                                                  model.online = 0;
...
//delete type cause
  deleteTypeCauseAction(context, position, typeCause) {
    AwesomeDialog(
        context: context,
        ...
        btnOk: ElevatedButton(
          ...
          onPressed: () async {
            await ActionService().deleteTypeCauseActionById(position).then(
                (resp) async {
              ShowSnackBar.snackBar(
