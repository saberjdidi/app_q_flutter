...
class CritereCheckListPage extends StatefulWidget {
  final CheckListAuditModel modelCheckList;
  final AuditModel modelAudit;
  const CritereCheckListPage(
      {Key? key, required this.modelCheckList, required this.modelAudit})
      : super(key: key);
...
class _CritereCheckListPageState extends State<CritereCheckListPage> {
  ...
  List<CritereChecklistAuditModel> listCritereChecklist =
      List<CritereChecklistAuditModel>.empty(growable: true);
  List<TextEditingController> controllersCritere = List.empty(growable: true);
  List<TextEditingController> controllersCommentaire =
  ...
  bool isVisibleBtnAddConstat = true;

  void getData() async {
    try {
      var connection = await Connectivity().checkConnectivity();
      if (connection == ConnectivityResult.none) {
        isVisibleBtnAddConstat = false;
        final response = await LocalAuditService().readCritereCheckListAudit(
            widget.modelAudit.refAudit, widget.modelCheckList.codeChamp);
        response.forEach((data) async {
          setState(() {
            var model = CritereChecklistAuditModel();
            model.online = data['online'];
            model.refAudit = data['refAudit'];
            model.idChamp = data['idChamp'];
            ...
            for (var i = 0; i < listCritereChecklist.length; i++) {
              ...
              controllersCommentaire[i].text =
                  listCritereChecklist[i].commentaire.toString();
              idCritere = listCritereChecklist[i].idCrit;
              debugPrint('id critere : $idCritere');
            }
          });
        });
      } else if (connection == ConnectivityResult.wifi ||
          connection == ConnectivityResult.mobile) {
        isVisibleBtnAddConstat = true;
        //rest api
        await AuditService()
            .getCritereOfCheckList(
                widget.modelAudit.refAudit, widget.modelCheckList.codeChamp, 1)
            .then((resp) async {
    ...
    try {
      ...
      if (connection == ConnectivityResult.none) {
        var response = await LocalAuditService()
            .readAuditeurInterne(widget.modelAudit.refAudit);
        ...
        //rest api
        await AuditService()
            .getAuditeurInterne(widget.modelAudit.refAudit)
            .then((response) async {
          //isDataProcessing(false);
          response.forEach((data) async {
            setState(() {
    ...
    try {
      ...
      if (connection == ConnectivityResult.none) {
        final response = await LocalAuditService()
            .readEmployeHabiliteAudit(widget.modelAudit.refAudit);
        response.forEach((data) async {
          setState(() {
        ...
        //rest api
        await AuditService()
            .getEmployeHabiliteAudit(widget.modelAudit.refAudit)
            .then((response) async {
  ...
  Future<void> getAuditByRefAudit() async {
    try {
      ...
      if (connection == ConnectivityResult.none) {
        final response = await LocalAuditService()
            .readConstatAuditByRefAudit(widget.modelAudit.refAudit);
        response.forEach((data) {
          setState(() {
            validation_constat = data['validation'];
          ...
          connection == ConnectivityResult.mobile) {
        await AuditService()
            .getAuditByRefAudit(widget.modelAudit.refAudit)
            .then((value) async {
      ...
      ShowSnackBar.snackBar(
          'Exception Audit Details', exception.toString(), Colors.red);
    }
  }
  ...
  void getMaxConstatAudit() async {
    try {
      ...
      if (connection == ConnectivityResult.none) {
        ...
        setState(() {
          maxConstatAudit = max_audit_offline;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text(
          '${'champ_audit'.tr} : ${widget.modelCheckList.champ}',
          ...
          child: listCritereChecklist.isNotEmpty
              ? ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    controllersCritere[index].text =
                        listCritereChecklist[index].critere.toString();
                    //controllersCommentaire.add(new TextEditingController());
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        title: TextFormField(
                          enabled: (presentAuditeurInterne ||
                            ...
                            Padding(
                              ...
                              child: DropdownSearch<ISPSPNCModel>(
                                ...
                                onChanged: (data) {
                                  ...
                                  critere = data?.value;
                                  listCritereChecklist[index].evaluation =
                        ...
                        trailing: Visibility(
                          visible: (presentAuditeurInterne ||
                                  presentEmployeHabilite) &&
                              enableValidationConstat &&
                              isVisibleBtnAddConstat,
                          child: Tooltip(
                            message: '${'new'.tr} Constat',
                            child: IconButton(
                              onPressed: () {
                                //----------------------Add Constat-----------------------------
                                ...
                                TextEditingController delaiRealController =
                                    TextEditingController();
                                ...
                                //type constat
                                Future<List<TypeAuditModel>> getTypeConstat(
                                    filter) async {
                                  try {
                                    List<TypeAuditModel> _typeList =
                                        await List<TypeAuditModel>.empty(
                                            growable: true);
                                ...
                                //gravite
                                Future<List<GraviteModel>> getGravite(
                                    filter) async {
                                  try {
                                    List<GraviteModel> _list =
                                        await List<GraviteModel>.empty(
                                            growable: true);
                                ...
                                //Employe
                                Future<List<EmployeModel>> getPersonneConcerne(
                                    filter) async {
                                  try {
                                    ...
                                    if (connection == ConnectivityResult.none) {
                                      ...
                                      //Get.snackbar("Internet Connection", "Mode Online", colorText: Colors.blue, snackPosition: SnackPosition.TOP);
                                      await AuditService()
                                          .getPersonConstatAudit(
                                              widget.modelAudit.refAudit,
                                ...
                                //champ audit
                                Future<List<ChampAuditModel>>
                                    getChampAuditByFiche(filter) async {
                                  try {
                                    List<ChampAuditModel> listType =
                                        await List<ChampAuditModel>.empty(
                                            growable: true);
                                    ...
                                    filterType = listType.where((u) {
                                      var code =
                                          u.codeChamp.toString().toLowerCase();
                                    ...
                                    if (kDebugMode) {
                                      print(
                                          'exception type audit : ${Exception.toString()}');
                                    }
                                    ShowSnackBar.snackBar(
                                ...
                                Widget _customDropDownChampAudit(
                                    BuildContext context,
                                    ChampAuditModel? item) {
                                  if (item == null) {
                                    return Container(
                                      child: ListTile(
                                        contentPadding: EdgeInsets.all(0),
                                        title: Text(
                                            '${widget.modelCheckList.champ}'),
                                      ),
                                    );
                                  } else {
                                    return Container(
                                ...
                                //type action
                                Future<List<TypeActionModel>> getTypeAction(
                                    filter) async {
                                  try {
                                    List<TypeActionModel> _typesList =
                                        await List<TypeActionModel>.empty(
                                            growable: true);
                                ...
                                Widget _customDropDownType(BuildContext context,
                                    TypeActionModel? item) {
                                  if (item == null) {
                                    return Container();
                                  } else {
                                    return Container(
                                                                    ...
                                                                    children: [
                                                                      Visibility(
                                                                        ...
                                                                        child:
                                                                          ...
                                                                          decoration: InputDecoration(
                                                                              ...
                                                                              border: OutlineInputBorder(borderSide: BorderSide(color: Colors.lightBlue, width: 1), borderRadius: BorderRadius.all(Radius.circular(10)))),
                                                                          validator: (value) =>
                                                                      ...
                                                                      Visibility(
                                                                          ...
                                                                          child:
                                                                            ...
                                                                            dropdownSearchDecoration:
                                                                                InputDecoration(
                                                                              labelText: "${'type_action_recommended'.tr} *",
                                                                            ...
                                                                            onChanged:
                                                                              ...
                                                                              debugPrint('type action: ${typeActionModel?.typeAct}, code: ${selectedCodeTypeAct}');
                                                                            },
                                                                            dropdownBuilder:
                                                                                _customDropDownType,
                                                                            popupItemBuilder:
                                                                      ...
                                                                      DropdownSearch<
                                                                        ...
                                                                        onChanged:
                                                                          ...
                                                                          selectedTypeConstat =
                                                                              data?.codeType;
                                                                          debugPrint(
                                                                              'type constat: ${constatAuditModel?.type}, num: ${selectedTypeConstat}');
                                                                        },
                                                                        dropdownBuilder:
                                                                      ...
                                                                      DropdownSearch<
                                                                        ...
                                                                        onChanged:
                                                                          ...
                                                                          selectedNGravite =
                                                                              data?.codegravite;
                                                                          debugPrint(
                                                                              'gravite: ${graviteModel?.gravite}, num: ${selectedNGravite}');
                                                                        },
                                                                        dropdownBuilder:
                                                                              ...
                                                                              SizedBox(
                                                                                ...
                                                                                child: DropdownSearch<EmployeModel>(
                                                                                  ...
                                                                                  onChanged: (data) {
                                                                                    ...
                                                                                    debugPrint('personne concernée: ${employeModel?.nompre}, mat: ${selectedMatriculeEmploye}');
                                                                                  },
                                                                                  dropdownBuilder: _customDropDownEmploye,
                                                                                  popupItemBuilder: _customPopupItemBuilderEmploye,
                                                                                  validator: (u) => u == null ? "${'person_concerne'.tr} ${'is_required'.tr} " : null,
                                                                                ),
                                                                              ),
                                                                              SizedBox(
                                                                                width: 5,
                                                                            ...
                                                                            children: <Widget>[
                                                                              SizedBox(
                                                                                ...
                                                                                child: DropdownSearch<EmployeModel>(
                                                                                  ...
                                                                                  dropdownBuilder: _customDropDownEmploye,
                                                                                  popupItemBuilder: _customPopupItemBuilderEmploye,
                                                                                  validator: (u) => u == null ? "Employe ${'is_required'.tr} " : null,
                                                                                ),
                                                                              ),
                                                                              SizedBox(
                                                                                width: 5,
                                                                              ...
                                                                              TextButton(
                                                                                  ...
                                                                                  child: Icon(Icons.west_outlined, size: 30, color: Color(0xFF1A6E84)))
                                                                            ],
                                                                          ),
                                                                        ),
                                                                      ),
                                                                      ...
                                                                      Row(
                                                                        ...
                                                                        children: [
                                                                          ConstrainedBox(
                                                                            constraints:
                                                                                BoxConstraints.tightFor(width: MediaQuery.of(context).size.width / 3, height: 50),
                                                                            child:
                                                                                ElevatedButton.icon(
                                                                              style: ButtonStyle(
                                                                                ...
                                                                                backgroundColor: MaterialStateProperty.all(CustomColors.firebaseRedAccent),
      ...
      floatingActionButton: FloatingActionButton.extended(
          onPressed: (presentAuditeurInterne || presentEmployeHabilite) &&
                  enableValidationConstat
              ? () async {
                  try {
                    for (var i = 0; i < listCritereChecklist.length; i++) {
                      debugPrint(
                          'checklist : ${listCritereChecklist[i].idCrit}-${listCritereChecklist[i].critere} -eval:${listCritereChecklist[i].evaluation} - comment:${controllersCommentaire[i].text}');
                      var connection = await Connectivity().checkConnectivity();
                      if (connection == ConnectivityResult.none) {
                        setState(() {
                          isProcessing = true;
                        ...
                        var model = CritereChecklistAuditModel();
                        model.online = 0;
                        model.refAudit = widget.modelAudit.refAudit;
                        model.idChamp = widget.modelCheckList.codeChamp;
                        model.idCrit = listCritereChecklist[i].idCrit;
                        model.critere = listCritereChecklist[i].critere;
                        model.evaluation = listCritereChecklist[i].evaluation;
                        model.commentaire = controllersCommentaire[i].text;

                        await LocalAuditService()
                            .updateCritereCheckListAudit(model);
            ...
            child: isExtended
                ...
                : Row(
                    children: [
                      ...
                      Text(
                        isProcessing ? 'processing'.tr : 'valider'.tr,
  ...
  Future<List<ISPSPNCModel>> getRespect(filter) async {
    try {
      List<ISPSPNCModel> evalList = [
        ISPSPNCModel(value: "0", name: 'not_evaluated'.tr),
        ISPSPNCModel(value: "1", name: 'conforme'.tr),
        ISPSPNCModel(value: "2", name: 'non_conforme'.tr)
      ];
  ...
  Widget _customDropDownRespect(BuildContext context, ISPSPNCModel? item) {
    if (item == null) {
      String? message_evaluation = 'not_evaluated'.tr;
      switch (eval) {
        case 0:
          message_evaluation = 'not_evaluated'.tr;
          break;
        case 1:
          message_evaluation = 'conforme'.tr;
          break;
        case 2:
          message_evaluation = 'non_conforme'.tr;
          break;
        default:
          message_evaluation = "";
      }
