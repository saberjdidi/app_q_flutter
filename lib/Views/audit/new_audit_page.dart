...
class NewAuditPage extends GetView<NewAuditController> {
                      ...
                      child: Column(
                        children: <Widget>[
                          ...
                          TextFormField(
                            ...
                            textInputAction: TextInputAction.next,
                            decoration: InputDecoration(
                              labelText: 'Objective Audit',
                              hintText: 'Objective Audit',
                              labelStyle: TextStyle(
                                fontSize: 14.0,
                              ),
                              hintStyle: TextStyle(
                          ...
                          Visibility(
                            ...
                            child: TextFormField(
                              ...
                              textInputAction: TextInputAction.next,
                              decoration: InputDecoration(
                                  labelText: 'Ref. ${'interne'.tr} ',
                                  hintText: 'Ref ${'interne'.tr}',
                                  labelStyle: TextStyle(
                                    fontSize: 14.0,
                                  ),
                                  hintStyle: TextStyle(
                          ...
                          DropdownSearch<TypeAuditModel>(
                            ...
                            onChanged: (data) {
                              ...
                              if (kDebugMode) {
                                print(
                                    'type audit : ${controller.typeAudit}, code : ${controller.selectedCodeTypeAudit}');
                              }
                            },
                          ...
                          Visibility(
                            visible: true,
                            child:
                                DropdownSearch<ChampAuditModel>.multiSelection(
                              ...
                              onChanged: (data) {
                                ...
                                data.forEach((element) {
                                  print(
                                      'champ audit: ${element.champ}, code: ${element.codeChamp}');
                                  List<int> listCodeChamp = [];
                                  List<String> listChamp = [];
                                  listCodeChamp.add(element.codeChamp!);
                                  controller.codeChampAuditList
                                      .addAll(listCodeChamp);
                                  listChamp.add(element.champ!);
                                  controller.champAuditList.addAll(listChamp);
                                });
                              },
                              dropdownBuilder: controller
                                  .customDropDownMultiSelectionChampAudit,
                              popupItemBuilder:
                                  controller.customPopupItemBuilderChampAudit,
                              popupSafeArea:
                          ...
                          Visibility(
                              ...
                              child: DropdownSearch<DirectionModel>(
                                  ...
                                  validator: (u) =>
                                      (controller.direction_obligatoire.value ==
                                                  1 &&
                          ...
                          Visibility(
                              ...
                              child: DropdownSearch<ActivityModel>(
                                  ...
                                  validator: (u) =>
                                      (controller.activity_obligatoire.value ==
                                                  1 &&
  ...
  Future<List<TypeAuditModel>> getType(filter) async {
    try {
      List<TypeAuditModel> listType =
          await List<TypeAuditModel>.empty(growable: true);
      List<TypeAuditModel> filterType =
          await List<TypeAuditModel>.empty(growable: true);
      ...
      if (connection == ConnectivityResult.none) {
        ...
        await AuditService().getTypeAudit(controller.matricule).then(
            (resp) async {
      ...
      filterType = listType.where((u) {
        ...
        return code.contains(filter) || description.contains(filter);
      }).toList();
      return filterType;
    } catch (Exception) {
      if (kDebugMode) {
      ...
      ShowSnackBar.snackBar(
          "Exception type audit", Exception.toString(), Colors.red);
    ...
    try {
      ...
      if (connection == ConnectivityResult.none) {
        ...
        response.forEach((element) {
          ...
          connection == ConnectivityResult.wifi) {
        await AuditService().getChampAudit(controller.matricule).then(
            (resp) async {
          resp.forEach((element) async {
            var model = ChampAuditModel();
            model.codeChamp = element['codeChamp'];
  ...
  Future<List<ProcessusModel>> getProcessus(filter) async {
    try {
      ...
      if (connection == ConnectivityResult.none) {
        //Get.snackbar("No Connection", "Mode Offline", colorText: Colors.blue, snackPosition: SnackPosition.TOP);
        var sites = await controller.localActionService
            .readProcessusByModule("Audit", "Audit");
        sites.forEach((data) {
    ...
    try {
      ...
      if (connection == ConnectivityResult.none) {
        ...
        var response = await controller.localActionService
            .readDirectionByModule("Audit", "Audit");
        response.forEach((data) {
      ...
      if (connection == ConnectivityResult.none) {
        ...
        var response = await controller.localActionService
            .readServiceByModuleAndDirection(
                'Audit', 'Audit', controller.selectedCodeDirection);
  ...
  Future<List<ActivityModel>> getActivity(filter) async {
    try {
      ...
      if (connection == ConnectivityResult.none) {
        //Get.snackbar("No Connection", "Mode Offline", colorText: Colors.blue, snackPosition: SnackPosition.TOP);
        var response = await controller.localActionService
            .readActivityByModule("Audit", "Audit");
        response.forEach((data) {
