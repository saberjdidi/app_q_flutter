...
class NewTypeConsequenceIncidentEnv extends StatefulWidget {
  ...
  const NewTypeConsequenceIncidentEnv({Key? key, required this.numIncident}) : super(key: key);

  @override
  State<NewTypeConsequenceIncidentEnv> createState() => _NewTypeConsequenceIncidentEnvState();
}

class _NewTypeConsequenceIncidentEnvState extends State<NewTypeConsequenceIncidentEnv> {
    ...
    if(_addItemFormKey.currentState!.validate()){
      try {
        ...
        if(connection == ConnectivityResult.none) {
          int max_num = await LocalIncidentEnvironnementService().getMaxNumTypeConsequenceIncidentEnvironnementRattacher();
          ...
          Get.to(TypeConsequenceIncidentEnvPage(numIncident: widget.numIncident,));
        }
  ...
  Future<List<TypeConsequenceIncidentModel>> getTypeConsequence(filter) async {
    try {
      ...
      if(connection == ConnectivityResult.none) {
        ...
        var response = await LocalIncidentEnvironnementService().readTypeConsequenceIncidentEnvARattacher(widget.numIncident);
        ...
        await IncidentEnvironnementService().getTypeConsequenceByIncidentARattacher(widget.numIncident, matricule).then((resp) async {
  ...
  Widget _customDropDownTypeConsequence(BuildContext context, TypeConsequenceIncidentModel? item) {
    if (item == null) {
      return Container();
    }
    else{
