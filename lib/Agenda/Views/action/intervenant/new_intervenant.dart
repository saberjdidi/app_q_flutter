...
class NewIntervenant extends StatefulWidget {
  ...
  const NewIntervenant({Key? key, required this.idAction, required this.idSousAction}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Action N° ${widget.idAction}",textAlign: TextAlign.center,),
      ...
      try {
        ...
        listEmployeSelected.forEach((element) async {
          ...
          await actionService.saveIntervenant(widget.idAction, widget.idSousAction, element.mat).then((resp) async {
            ShowSnackBar.snackBar("Intervenant Successfully", "${element.mat} added", Colors.green);
            //Get.back();
            Get.to(IntervenantsActionRealisationPage(idAction: widget.idAction, idSousAction: widget.idSousAction,));
          }, onError: (err) {
            _isProcessing = false;
