...
class RemplirPNCTraitement extends StatefulWidget {
  ...
  RemplirPNCTraitement({Key? key, required this.pncTraiterModel}) : super(key: key);
...
class _RemplirPNCTraitementState extends State<RemplirPNCTraitement> {
  ...
  bool _dataValidation(){
    ...
    if(responsableTraitementModel == null &&  responsableTraitementId == 0){
      Message.taskErrorOrWarning("Alert", "Veuillez selectionner responsable de traitement");
      return false;
    }
    if(depreciation > 100){
      Message.taskErrorOrWarning("Alert", "Veuillez saisir donnée inférieur ou égal à 100");
      return false;
    }
    if((quantity_accepte + quantity_declasse + quantity_rejete) > quantiy_detect){
      Message.taskErrorOrWarning("Alert", "La somme des quantités ne doit pas dépasser la quantité détectée");
      return false;
    }
                          ...
                          children: <Widget>[
                            ...
                            DropdownSearch<ResponsableTraitementModel>(
                              ...
                              onChanged: (data) {
                                ...
                                print('resp traitement: ${data?.nompre}, id: ${responsableTraitementId}');
                              },
                              ...
                              children: [
                                Row(
                                  children: [
                                    ...
                                    const Text("Phase non traitée", style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Colors.black),)
                                  ],
                                ),
                                Row(
                                  children: [
                                    Radio(value: 1,
                                      ...
                                      activeColor: Colors.blue,
                                      fillColor: MaterialStateProperty.all(Colors.blue),),
                                    const Text("Phase traitée", style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Colors.black),)
                                  ],
                                )
                              ],
                            ),
  ...
  Future saveBtn() async {
    if(_dataValidation() && _addItemFormKey.currentState!.validate()){
      try {
        ...
        await pncService.getProductsOfPNCTraiter(widget.pncTraiterModel.nnc)
            .then((resp) async {
        ...
        await pncService.remplirPNCTraitement2({
          ...
          ShowSnackBar.snackBar("Successfully", "PNC of ${responsableTraitementModel?.nompre} Traitee", Colors.green);
          //Get.back();
          Get.to(PNCTraiterPage());
  ...
  Future<List<ResponsableTraitementModel>> getResponsableTraitement(filter) async {
    try {
      List<ResponsableTraitementModel> employeList = await List<ResponsableTraitementModel>.empty(growable: true);
      List<ResponsableTraitementModel>employeFilter = await List<ResponsableTraitementModel>.empty(growable: true);

      await PNCService().getResponsableTraitementByNNC(widget.pncTraiterModel.nnc).then((resp) async {
        resp.forEach((data) async {
  ...
  Widget customDropDownEmploye(BuildContext context, ResponsableTraitementModel? item) {
    if (item == null) {
      return Container();
    }
    else{
