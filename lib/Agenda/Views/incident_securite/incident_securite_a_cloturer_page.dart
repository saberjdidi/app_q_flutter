...
import 'valider_incident_securite_cloturer.dart';

class IncidentSecuriteACloturerPage extends StatefulWidget {

  IncidentSecuriteACloturerPage({Key? key}) : super(key: key);

  @override
  State<IncidentSecuriteACloturerPage> createState() => _IncidentSecuriteACloturerPageState();
}

class _IncidentSecuriteACloturerPageState extends State<IncidentSecuriteACloturerPage> {
    ...
    try {
      ...
      if(connection == ConnectivityResult.none) {
       // Get.snackbar("No Connection", "Mode Offline", colorText: Colors.blue, snackPosition: SnackPosition.BOTTOM, duration: Duration(milliseconds: 900));
        var response = await LocalIncidentSecuriteService().readIncidentSecuriteACloturer();
       ...
       //rest api
        await IncidentSecuriteService().getListIncidentSecuriteACloturer(matricule).then((response) async {
  ...
  List<DataRow> getRows(List<IncidentSecuriteAgendaModel> listIncidentInfo) => listIncidentInfo.map((IncidentSecuriteAgendaModel incidentSecuriteAgendaModel) {
    return DataRow(cells: [
      DataCell(GestureDetector(
        onTap: ()=> Get.to(ValiderIncidentSecuriteCloturer(model: incidentSecuriteAgendaModel)),
        child: Text('${incidentSecuriteAgendaModel.ref}', style: _contentStyle, textAlign: TextAlign.right),
      )),
      DataCell(Text('${incidentSecuriteAgendaModel.designation}', style: _contentStyle, textAlign: TextAlign.right)),
      DataCell(Text('${incidentSecuriteAgendaModel.dateInc}', style: _contentStyle, textAlign: TextAlign.right)),
      DataCell(
          InkWell(
              onTap: (){
                Get.to(ValiderIncidentSecuriteCloturer(model: incidentSecuriteAgendaModel));
              },
