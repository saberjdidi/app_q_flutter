...
class DecisionTraitementIncidentSecuritePage extends StatefulWidget {
  ...
  @override
  State<DecisionTraitementIncidentSecuritePage> createState() => _DecisionTraitementIncidentSecuritePageState();
}

class _DecisionTraitementIncidentSecuritePageState extends State<DecisionTraitementIncidentSecuritePage> {
    ...
    try {
      ...
      if(connection == ConnectivityResult.none) {
       // Get.snackbar("No Connection", "Mode Offline", colorText: Colors.blue, snackPosition: SnackPosition.BOTTOM, duration: Duration(milliseconds: 900));
        var response = await LocalIncidentSecuriteService().readIncidentSecuriteDecisionTraitement();
       ...
       //rest api
        await IncidentSecuriteService().getListIncidentSecuriteDecisionTraitement(matricule).then((response) async {
  ...
  List<DataRow> getRows(List<IncidentSecuriteAgendaModel> listIncidentInfo) => listIncidentInfo.map((IncidentSecuriteAgendaModel incidentSecuriteAgendaModel) {
    return DataRow(cells: [
      DataCell(InkWell(
        onTap: (){
          Get.to(ValiderIncidentSecuriteDecisionTraitement(model: incidentSecuriteAgendaModel));
        },
