...
class RapportAuditAValiderPage extends StatefulWidget {
  ...
  @override
  State<RapportAuditAValiderPage> createState() =>
      _RapportAuditAValiderPageState();
}
  ...
  getData() async {
    try {
      ...
      if (connection == ConnectivityResult.none) {
        ...
        await AuditService().getRapportAuditsAValider().then((response) {
          response.forEach((element) {
            setState(() {
              var model = new AuditModel();
              model.idAudit = element['idAudit'];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(
          '${'rapport_audit_a_valider'.tr} : ${listAudit.length}',
                              ...
                              return Card(
                                ...
                                child: ListTile(
                                        ...
                                        RichText(
                                          text: TextSpan(
                                            ...
                                            children: [
                                              ...
                                              TextSpan(
                                                  text:
                                                      '${filterAudit[index].typeA}'),

                                              //TextSpan(text: '${action.declencheur}'),
                                            ],
                                          ),
                                        ),
                                        filterAudit[index].interne == ''
                                            ? Text('')
                                            : Text(
                                  ...
                                  trailing: InkWell(
                                    onTap: () {
                                      Get.to(
                                          ValiderRapportAuditPage(
                                            model: filterAudit[index],
                                          ),
                                          transition: Transition.cupertino);
                                    },
                                    child: Icon(
                                      Icons.edit,
                                      color: Color(0xDF0D70B3),
                                    ),
                                  ),
                                  onTap: () {
                                    Get.to(
