...
class ReunionInformerPage extends StatefulWidget {
  ReunionInformerPage({Key? key}) : super(key: key);

  @override
  State<ReunionInformerPage> createState() => _ReunionInformerPageState();
}

class _ReunionInformerPageState extends State<ReunionInformerPage> {
  ...
  void getReunion() async {
    try {
      ...
      if (connection == ConnectivityResult.none) {
        ...
        var response = await LocalReunionService().readReunionInformer();
  ...
  final columns = ['number'.tr, 'Type', 'Date', 'heur'.tr, ''];
  int? sortColumnIndex;
    ...
    return Container(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(
            '${'reunion'.tr} ${'pour'.tr} Info : ${listReunion.length}',
  ...
  List<DataRow> getRows(List<ReunionModel> listReunionInfo) =>
      listReunionInfo.map((ReunionModel reunionModel) {
        return DataRow(
            cells: [
              DataCell(InkWell(
                onTap: () {
                  Get.to(ConfirmerReunionInformer(model: reunionModel));
                },
                child: Text('${reunionModel.nReunion}',
                    style: _contentStyle, textAlign: TextAlign.right),
              )),
              DataCell(Text('${reunionModel.typeReunion}',
                  style: _contentStyle, textAlign: TextAlign.right)),
        ...
        moreStyle: TextStyle(
            ...
            color: CustomColors.bleuCiel),
              ...
              DataCell(Text('${reunionModel.datePrev}',
                  style: _contentStyle, textAlign: TextAlign.right)),
              DataCell(Text('${reunionModel.heureDeb}',
                  style: _contentStyle, textAlign: TextAlign.right)),
              DataCell(InkWell(
                  onTap: () {
                    Get.to(ConfirmerReunionInformer(model: reunionModel));
  ...
  void onSort(int columnIndex, bool ascending) {
    if (columnIndex == 0) {
      listReunion.sort((value1, value2) => compareString(
          ascending, value1.nReunion.toString(), value2.nReunion.toString()));
    } else if (columnIndex == 1) {
      listReunion.sort((value1, value2) => compareString(ascending,
          value1.typeReunion.toString(), value2.typeReunion.toString()));
    } else if (columnIndex == 2) {
      listReunion.sort((value1, value2) => compareString(
          ascending, value1.datePrev.toString(), value2.datePrev.toString()));
    }
    setState(() {
