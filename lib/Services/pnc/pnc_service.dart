...
class PNCService {
  ...
  Future<dynamic> uploadImagePNC(Map data) async {
    try {
      debugPrint('data : ${json.encode(data)}');
      var response = await http.post(
          Uri.parse('${AppConstants.UPLOAD_URL}/uploadPhotoPNC'),
          body: json.encode(data),
          headers: {'Content-Type': 'application/json'});
      if (response.statusCode == 200) {
        return await jsonDecode(response.body);
  ...
  Future<List<dynamic>> getImagePNC(idFiche) async {
    try {
      debugPrint('${AppConstants.UPLOAD_URL}/getImagePNC?idFiche=$idFiche');
      var response = await http.get(
          Uri.parse('${AppConstants.UPLOAD_URL}/getImagePNC?idFiche=$idFiche'),
          headers: {'Content-Type': 'application/json'});
  ...
  Future<List<dynamic>> getDecideurPNC(site, processus) async {
    try {
      var response = await http.get(
          Uri.parse(
              '${AppConstants.PNC_URL}/listDecideur?site=$site&processus=$processus'),
          headers: {'Content-Type': ' application/json'});
      print('response : ${jsonDecode(response.body)}');
      if (response.statusCode == 200) {
        return await jsonDecode(response.body);
  ...
  Future<List<dynamic>> getProductsPNC(nnc, matricule) async {
    try {
      var response = await http.get(
          Uri.parse(
              '${AppConstants.PNC_URL}/listProduitNc?nnc=$nnc&mat=$matricule'),
          headers: {'Content-Type': ' application/json'});
      //print(AppConstants.ACTION_URL);
      //print('response ${jsonDecode(response.body)}');
      if (response.statusCode == 200) {
  ...
  Future<List<dynamic>> getProductsPNCARattacher(nnc, mat) async {
    try {
      var response = await http.get(
          Uri.parse(
              '${AppConstants.PNC_URL}/listProductNNCARattacher?nnc=$nnc&numPage=300&mat=$mat'),
          headers: {'Content-Type': ' application/json'});
      //print(AppConstants.ACTION_URL);
      //print('response ${jsonDecode(response.body)}');
      if (response.statusCode == 200) {
  ...
  //type product nc
  Future<List<dynamic>> getTypeProductNC(nnc, product, mat) async {
    try {
      var response = await http.get(
          Uri.parse(
              '${AppConstants.PNC_URL}/listTypeNc?nnc=$nnc&idProduit=$product&mat=$mat'),
          headers: {'Content-Type': 'application/json'});
      if (response.statusCode == 200) {
        return await jsonDecode(response.body);
      } else {
  ...
  Future<List<dynamic>> getResponsableTraitementByNNC(nnc) async {
    try {
      ...
      var response = await http.get(
          Uri.parse(
              '${AppConstants.PNC_URL}/listNcRresponsableTraitement?nnc=$nnc'),
          headers: {'Content-Type': ' application/json'});
      print('response : ${jsonDecode(response.body)}');
      if (response.statusCode == 200) {
        return await jsonDecode(response.body);
  ...
  Future<List<dynamic>> getPNCACorriger(matricule) async {
    try {
      var response = await http.get(
          Uri.parse(
              '${AppConstants.PNC_URL}/liste_nc_a_corriger?mat=$matricule'),
          headers: {'Content-Type': ' application/json'});
      if (response.statusCode == 200) {
        return await jsonDecode(response.body);
      } else {
  ...
  Future<List<dynamic>> getPNCInvestigationEffectuer(matricule) async {
    try {
      var response = await http.get(
          Uri.parse(
              '${AppConstants.PNC_URL}/listeInvestigationEffectue?mat=$matricule'),
          headers: {'Content-Type': 'application/json'});
      if (response.statusCode == 200) {
        return await jsonDecode(response.body);
      } else {
  ...
  Future<List<dynamic>> getPNCInvestigationApprouver(matricule) async {
    try {
      var response = await http.get(
          Uri.parse(
              '${AppConstants.PNC_URL}/listInvestigation_a_approuver?mat=$matricule'),
          headers: {'Content-Type': ' application/json'});
      if (response.statusCode == 200) {
        return await jsonDecode(response.body);
      } else {
  ...
  Future<List<dynamic>> getPNCTraitementDecision(matricule) async {
    try {
      debugPrint(
          '${AppConstants.PNC_URL}/getlisteTraitementDecision?mat=$matricule');
      var response = await http.get(
          Uri.parse(
              '${AppConstants.PNC_URL}/getlisteTraitementDecision?mat=$matricule'),
          headers: {'Content-Type': 'application/json'});
  ...
  Future<List<dynamic>> getPNCValiderDecisionTraitement(matricule) async {
    try {
      var response = await http.get(
          Uri.parse(
              '${AppConstants.PNC_URL}/ListeDecisionTraitementValider?mat=$matricule'),
          headers: {'Content-Type': 'application/json'});
      if (response.statusCode == 200) {
        return await jsonDecode(response.body);
      } else {
