...
class ActionService extends GetConnect {
  // Fetch Data
  Future<List<ActionModel>> getAction(
      numAction, action, matricule, type) async {
    ...
    try {
      var response =
          await http.post(Uri.parse('${AppConstants.ACTION_URL}/ListeAction'),
              body: json.encode({
                "nact": numAction,
                "act": action,
                "refaud": "",
      ...
      if (response.statusCode == 200) {
        ...
        return result.map(((e) => ActionModel.fromJson(e))).toList();
      } else {
  ...
  Future<dynamic> saveAction(Map data) async {
    try {
      ...
      debugPrint('${Uri.parse('${AppConstants.ACTION_URL}/addAction')}');
      var response = await http.post(
  ...
  Future<List<dynamic>> getSousAction(id) async {
    ...
    try {
      var response = await http.get(
          Uri.parse(
              '${AppConstants.SOUS_ACTION_URL}/getListeSousAction?nac=$id'),
          headers: {'Content-Type': ' application/json'});
      //print('response ${jsonDecode(response.body)}');
      if (response.statusCode == 200) {
        return await jsonDecode(response.body);
  ...
  Future<List<dynamic>> getIntervenant(id_action, id_sous_action) async {
    print(
        '${AppConstants.ACTION_URL}/listeEmployesIntervenants?nact=$id_action&nsact=$id_sous_action');
    try {
      var response = await http.post(
          Uri.parse(
              '${AppConstants.ACTION_URL}/listeEmployesIntervenants?nact=$id_action&nsact=$id_sous_action'),
  ...
  Future<dynamic> saveIntervenant(nact, nsact, liste) async {
    try {
      print(
          'url : ${AppConstants.ACTION_URL}/AjoutListeIntervenants?nact=$nact&nsact=$nsact&liste=$liste');
      var response = await http.post(
          Uri.parse(
              '${AppConstants.ACTION_URL}/AjoutListeIntervenants?nact=$nact&nsact=$nsact&liste=$liste'),
          headers: {'Content-Type': ' application/json'});
      //print(AppConstants.ACTION_URL);
      print('response ${jsonDecode(response.body)}');
      if (response.statusCode == 200) {
  ...
  Future<dynamic> saveActionRealisationOfProject(Map data) async {
    try {
      ...
      var response = await http.post(
          Uri.parse(
              '${AppConstants.ACTION_URL}/RemplirActionRealisationFicheProjet'),
          body: json.encode(data),
          headers: {'Content-Type': ' application/json'});
      //print(AppConstants.ACTION_URL);
      print('response ${jsonDecode(response.body)}');
  ...
  Future<dynamic> getIdProjectOfAction(num_action) async {
    try {
      print(
          'url : ${AppConstants.ACTION_URL}/getProjectID?NumAction=$num_action');
      var response = await http.get(
          Uri.parse(
              '${AppConstants.ACTION_URL}/getProjectID?NumAction=$num_action'),
          headers: {'Content-Type': ' application/json'});
      //print(AppConstants.ACTION_URL);
      print('response ${jsonDecode(response.body)}');
      if (response.statusCode == 200) {
  ...
  Future<dynamic> saveActionRealisation2(
      commentaire, pourcent_real, depense, n_sous_act, n_act) async {
    try {
      print(
          'url : ${AppConstants.ACTION_URL}/remplir_action_realisation?Commentaire=$commentaire&PourcentReal=$pourcent_real&depense=$depense&NSousAct=$n_sous_act&NAct=$n_act');
      var response = await http.post(
          Uri.parse(
              '${AppConstants.ACTION_URL}/remplir_action_realisation?Commentaire=$commentaire&PourcentReal=$pourcent_real&depense=$depense&NSousAct=$n_sous_act&NAct=$n_act'),
          headers: {'Content-Type': ' application/json'});
      //print(AppConstants.ACTION_URL);
      print('response ${jsonDecode(response.body)}');
      if (response.statusCode == 200) {
  ...
  Future<List<dynamic>> getActionSuivi(Map data) async {
    debugPrint('data : ${json.encode(data)}');
    debugPrint('${AppConstants.SOUS_ACTION_URL}/GetAction_suivie');
    try {
      var response = await http.post(
  ...
  Future<List<dynamic>> getProductsOfAction(mat, num_action) async {
    try {
      var response = await http.get(
          Uri.parse(
              '${AppConstants.ACTION_URL}/produitByNactMat?Mat=$mat&NumAction=$num_action'),
          headers: {'Content-Type': ' application/json'});
      if (response.statusCode == 200) {
        return await jsonDecode(response.body);
      } else {
  ...
  Future<List<dynamic>> getTypesCausesOfAction(mat, num_action, online) async {
    try {
      var response = await http.get(
          Uri.parse(
              '${AppConstants.ACTION_URL}/typeCauseActionByNact?Mat=$mat&NumAction=$num_action&online=$online'),
          headers: {'Content-Type': 'application/json'});
      if (response.statusCode == 200) {
        return await jsonDecode(response.body);
      } else {
  ...
  Future<List<dynamic>> getTypesCauseActionARattacher(mat, num_action) async {
    try {
      var response = await http.get(
          Uri.parse(
              '${AppConstants.ACTION_URL}/listTypeCauseARattacher?mat=$mat&numAction=$num_action'),
          headers: {'Content-Type': 'application/json'});
      if (response.statusCode == 200) {
        return await jsonDecode(response.body);
      } else {
  ...
  Future<dynamic> saveTypeCauseAction(int? nAct, int? codeTypeCause) async {
    try {
      debugPrint(
          '${AppConstants.ACTION_URL}/ajoutTypeCauseAction?nAction=$nAct&codeTypeCause=$codeTypeCause');
      var response = await http.post(
          Uri.parse(
              '${AppConstants.ACTION_URL}/ajoutTypeCauseAction?nAction=$nAct&codeTypeCause=$codeTypeCause'),
          headers: {'Content-Type': 'application/json'});
